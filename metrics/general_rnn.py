...
class GeneralRNN():
  ...
  def fit(self, x, y, valid_rate = 0.2):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
      #my_tmpdir = '/data/nips_hns/hide-and-seek/tmp'
      save_file_name = os.path.join(tmpdir, 'model.{epoch:02d}.h5')
      ...
      self.predictor_model.fit(train_x, train_y, 
                               ...
                               callbacks=[save_best], verbose=0)

      best_file = os.path.join(tmpdir, sorted(os.listdir(tmpdir), reverse=True)[0])
      self.predictor_model.load_weights(best_file)
